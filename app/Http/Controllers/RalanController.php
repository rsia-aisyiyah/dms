...
class RalanController extends Controller
{
	...
	public function json(Request $request)
	{
		...
		if ($request->tgl_pertama && $request->tgl_kedua) {
			$data = $data->whereBetween('tgl_registrasi', [$request->tgl_pertama, $request->tgl_kedua]);
		}

		if ($request->stts_daftar) {
			$data = $data->where('stts_daftar', $request->stts_daftar);
		}

		if ($request->pembiayaan) {
			$data = $data->whereHas('penjab', function ($query) use ($request) {
				return $query->where('png_jawab', 'like', "%" . $request->pembiayaan . "%");
			});
		}

		if ($request->poli) {
			$data = $data->whereHas('dokter', function ($query) use ($request) {
				return $query->where('kd_sps', $request->poli);
			});
		}
