<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

class AddForeignKeysToDetailhibahObatBhpTable extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::table('detailhibah_obat_bhp', function (Blueprint $table) {
            $table->foreign(['kode_brng'], 'detailhibah_obat_bhp_ibfk_1')->references(['kode_brng'])->on('databarang')->onUpdate('CASCADE')->onDelete('CASCADE');
            $table->foreign(['kode_sat'], 'detailhibah_obat_bhp_ibfk_2')->references(['kode_sat'])->on('kodesatuan')->onUpdate('CASCADE');
            $table->foreign(['no_hibah'], 'detailhibah_obat_bhp_ibfk_3')->references(['no_hibah'])->on('hibah_obat_bhp')->onUpdate('CASCADE')->onDelete('CASCADE');
        });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::table('detailhibah_obat_bhp', function (Blueprint $table) {
            $table->dropForeign('detailhibah_obat_bhp_ibfk_1');
            $table->dropForeign('detailhibah_obat_bhp_ibfk_2');
            $table->dropForeign('detailhibah_obat_bhp_ibfk_3');
        });
    }
}
